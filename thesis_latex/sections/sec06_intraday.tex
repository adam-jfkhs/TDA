...
Apply Vietoris-Rips filtration to the distance matrix using the ripser library \citep{Tralie2018}. Extract $H_0$ (connected components) and $H_1$ (loops) persistence diagrams. For each diagram, record:
\begin{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase1_intraday/figure_6_2_h1_evolution.pdf}
...
This adaptive framework addresses both the sample size limitation (via intraday data) and the regime mismatch problem (via strategy switching) simultaneously, potentially improving performance where the fixed mean-reversion approach fails.
