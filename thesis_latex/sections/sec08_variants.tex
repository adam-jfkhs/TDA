...
\begin{quote}
\noindent\textbf{Projection Disclosure.} The Sharpe ratios reported in this section ($+0.42$ Momentum+TDA, $+0.18$ Scale-Consistent, $+0.35$ Adaptive Threshold, $+0.48$ Ensemble) are \textbf{projected outcomes} from the strategy designs in \texttt{thesis\_expansion/phase3\_strategy\_variants/}. The scripts also inherit the look-ahead bias identified in Section~\ref{sec:sector} (Phase 2): signals at date $t$ are applied to returns at date $t$ rather than $t+1$. The strategy logic is presented as an exploration of design choices; the numerical results were not reproduced under the stricter verification methodology applied in Section~\ref{sec:conclusion}. Treat figures below as upper bounds on what these designs could achieve; the only verified result is Strategy D (Sharpe $-0.77$) in the conclusion.
\end{quote}

\subsection{Motivation}

Sections 6--7 demonstrated that sector-specific topology substantially improves performance (initial estimate: Sharpe $+0.79$; verified estimate: $-0.77$; see Section~\ref{sec:conclusion}) compared to the cross-sector baseline ($-0.56$). However, this addressed only one of three primary failure modes identified in Section 5:

\begin{enumerate}
\item \checkmark \textbf{Correlation heterogeneity} $\rightarrow$ Solved by sector-specific analysis (Section 7)
\item $\times$ \textbf{Scale mismatch} $\rightarrow$ Daily signals filtered by monthly topology remain unaddressed
\item $\times$ \textbf{Mean-reversion incompatibility} $\rightarrow$ Strategy assumes mean reversion, but 2022--2024 markets trended
\end{enumerate}
...
\textbf{2. Strategy Design} (from Section~\ref{sec:variants}):
\begin{itemize}
...
This interpretation aligns with regime-switching literature \citep{Hamilton1989, Ang2002} but uses topological features instead of Hidden Markov Models.

\subsection{Conclusion}

Alternative strategy variants show initial Sharpe estimates of +0.18 to +0.48 across multiple design choices. However, like the Section~\ref{sec:sector} results, these estimates are subject to the test-window and signal-lag biases identified in Section~\ref{sec:conclusion} and require independent verification:

\begin{enumerate}
\item \textbf{Momentum + TDA Hybrid}: best initial standalone estimate (Sharpe +0.42), addressing mean-reversion failure in trending markets.

\item \textbf{Adaptive Threshold}: modest initial improvement (Sharpe +0.35) via dynamic regime detection.
...
\item \textbf{Ensemble Portfolio}: best initial estimate (Sharpe +0.48) through diversification across variants.
\end{enumerate}

These results show that variant choice is not the bottleneck---multiple design approaches produce similar outcomes. However, given that Section~\ref{sec:sector}'s initial Sharpe of $+0.79$ verified at $-0.77$, the absolute performance levels reported here should be treated as upper bounds.
...
\midrule
Baseline (Section~\ref{sec:preliminary}) & Sharpe $-0.56$ & Cross-sector mean-reversion \\
Phase 1 (Section~\ref{sec:intraday}) & Sharpe $-0.41$ & Intraday data (sample size) \\
Phase 2 (Section~\ref{sec:sector}) & Sharpe $+0.79$ (initial); $-0.77$ (verified) & Sector-specific (homogeneity) \\
Phase 3 (Section~\ref{sec:variants}) & Sharpe $+0.48$ (initial) & Strategy variants (robustness) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Next Phase}: Section~\ref{sec:crossmarket} tests external validity by applying sector-specific topology to international equities, cryptocurrencies, and commodities. If results generalize across asset classes, we establish topological trading as a robust, market-agnostic methodology.
