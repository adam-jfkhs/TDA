...
After six phases of empirical testing, algorithmic refinement, theoretical investigation, and independent verification, the answer is clear:

\begin{quote}
\textbf{No strategy tested produced positive risk-adjusted returns in a fully verified, look-ahead-free backtest. However, topology provides measurable structural insight into market regimes, and the mathematical framework developed here represents a genuine contribution to understanding correlation networks.}
\end{quote}

\subsubsection{Verified Backtest Results}

An independent verification backtest (\texttt{risk\_report.py}) was conducted using real Yahoo Finance data, proper 1-day signal lag, and Fama-French factor regression to control for known risk premia. Four strategies were tested on a 2022--2024 out-of-sample period with 5 basis point transaction costs:

\begin{table}[H]
\centering
\caption{Verified Out-of-Sample Performance (2022--2024)}
\label{tab:verified-backtest}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Strategy} & \textbf{Ann. Return} & \textbf{Sharpe} & \textbf{Max DD} & \textbf{Factor $\alpha$ (t-stat)} \\
\midrule
A: Laplacian + Fiedler filter  & $-30.8\%$ & $-2.57$ & $-61.6\%$ & $-34.98\%$ ($-5.13$) \\
B: Laplacian + Vol filter      & $-26.9\%$ & $-2.42$ & $-56.3\%$ & $-31.48\%$ ($-5.24$) \\
C: Laplacian only              & $-37.9\%$ & $-2.75$ & $-69.0\%$ & $-42.17\%$ ($-5.66$) \\
D: Beta-spread + Fiedler       & $-9.8\%$  & $-0.77$ & $-40.4\%$ & $-14.00\%$ ($-1.88$) \\
SPY benchmark                  & $+8.6\%$  & $+0.49$ & $-26.2\%$ & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
\item Strategies A, B, C (Laplacian mean-reversion) generate highly significant negative alpha ($t \approx -5$). The signal actively destroys value.
\item Strategy D (beta-spread with topology regime filter, replicating Section 7 logic) achieves Sharpe $-0.77$ --- substantially better, but still negative.
\item Strategy D's factor alpha ($t = -1.88$) is borderline and not statistically significant at $p < 0.05$. This means the result is not definitively negative either --- the strategy may break even in different market conditions.
\item The Fiedler-value filter reduces losses relative to no filter (Strategy A vs C), confirming topology's value as a \textbf{risk indicator}. However, a simple realized-volatility filter (Strategy B) achieves a better result at lower computational cost.
\end{itemize}

\subsubsection{Why Earlier Sharpe Estimates Were Optimistic}

Section 7's reported Sharpe of $+0.79$ reflects three compounding advantages not present in the verification backtest:

\begin{enumerate}
\item \textbf{Favorable test window}: Section 7 tests on 2023--2024 (bull market). The verification test includes 2022, a severe bear market, in the out-of-sample period.
\item \textbf{Larger stock universe}: Section 7 uses 20 stocks per sector; the verification uses 6. More stocks improve beta separation and reduce noise in the spread.
\item \textbf{Subtle look-ahead}: The original phase 2 code applied today's topology signal to today's returns. The verification code imposes a strict 1-day lag.
\end{enumerate}

This is not data fraud --- it is a common methodological pitfall in quantitative finance research \citep{Bailey2014, dePrado2018}. The verification exercise is itself a contribution: it demonstrates the importance of independent replication with clean methodology.

\subsubsection{What the Mathematical Framework Genuinely Contributes}

Although the trading strategy does not produce positive returns, the theoretical contributions of Sections 9--11 remain valid independent of strategy performance:
...
\item \textbf{Stable topology} $\rightarrow$ predictable regime signals (though not sufficient for positive alpha)
\end{enumerate}

The theoretical bound derived in Section 11:
\begin{equation}
\textbf{CV}(H_1) \leq \frac{\alpha}{\sqrt{\rho(1-\rho)}}
\end{equation}
remains mathematically valid and grounded in random matrix theory and spectral graph analysis. It explains \textit{when} topology is measurable --- even if measurable topology does not guarantee tradeable alpha.
...
\textbf{Practical Implication}: Use topology for \textbf{risk overlays} (regime detection, exposure scaling) rather than \textbf{pure alpha generation} (directional bets). Even the best-performing strategy variant ($-0.77$ Sharpe, verified) loses money, confirming that regime detection alone does not generate positive returns.
...
\begin{itemize}
\item High-correlation eigenvalues (empirical $\lambda_1 \approx 25.5$ for our $Q = T/n = 20$ universe) exceed the Marchenko-Pastur upper bound ($\lambda_+ \approx 1.50$) by more than an order of magnitude
...
\begin{itemize}
\item \citet{Gidea2018}: TDA detects crashes retrospectively (no trading strategy)
\item \citet{Meng2021}: Network topology descriptive analysis (no profitability test)
\item \citet{Macocco2023}: TDA + ML for crypto (limited validation)
\end{itemize}

\textbf{Our Four Contributions}:
\begin{enumerate}
\item \checkmark \textbf{First rigorous negative-result study} of TDA trading strategies with factor regression and look-ahead-free verification (Sharpe $-0.77$ best case, not profitable)
...
\item \textbf{Can the CV bound be tightened?}
   \begin{itemize}
   \item Current: CV $\leq \alpha/\sqrt{\rho(1-\rho)}$ with empirical $\alpha \approx 0.35$
...
\begin{itemize}
\item Build \textbf{Neural Network} with topology + correlation features (Section 10 architecture; best-performing model in our tests)
...
This thesis set out to answer whether topology can generate profitable trading signals. The verified answer is \textbf{no} --- none of the strategies tested produced positive risk-adjusted returns under clean out-of-sample conditions.

This is not a failure of the research. Rigorous negative results are a contribution to knowledge. The field of quantitative finance is littered with strategies that appear profitable in initial backtests and fail under scrutiny \citep{Harvey2016, Bailey2014}. This thesis demonstrates that scrutiny --- factor regressions, look-ahead-free methodology, independent verification --- and reports honestly what it finds.

\textbf{What remains true}: The correlation-stability mechanism (CV $\leq \alpha/\sqrt{\rho(1-\rho)}$) is mathematically valid. Sector homogeneity does produce more stable topology. The Fiedler value does correlate $-0.991$ with topology CV. Topology does provide a measurable risk signal that reduces drawdowns, even if it cannot overcome transaction costs and adverse market regimes to produce positive net returns.

\textbf{For practitioners}: Use topology (or equivalently, the Fiedler value of the correlation graph) as a \textbf{risk indicator} to scale exposure on strategies that already work. Do not use it as a standalone signal generator.

\textbf{For researchers}: The mathematical framework developed here --- spectral gap, CV bound, ghost loop detection --- provides tools for understanding when topological features are reliable signals versus noise. Proving (or disproving) the CV bound rigorously remains an open and worthwhile problem.

The shape of markets is real and measurable. Measuring it accurately is harder than generating alpha from it.
