...
\begin{quote}
\noindent\textbf{Verification Notice.} The Sharpe ratios reported in this section ($+0.79$ aggregate; $+0.87$ Financials; $+0.79$ Energy; $+0.68$ Technology) are \textit{pre-verification estimates} from the original Phase 2 analysis (\texttt{thesis\_expansion/phase2\_sector\_topology/}). An independent verification backtest with strict 1-day signal lag, 2022 included in out-of-sample, and Fama-French 5-factor regression yields Sharpe $-0.77$ for the closest replication (Strategy D in \texttt{risk\_report.py}). See Section~\ref{sec:conclusion} for the verified results table. The correlation-stability mechanism (CV vs $\rho$ relationship) reported below is independent of the trading-strategy claim and remains valid.
\end{quote}
...
\textbf{Causality}: High correlation $\rightarrow$ eigenvalue concentration $\rightarrow$ stable topology $\rightarrow$ reliable regime signals. However, as independent verification demonstrates (Section~\ref{sec:conclusion}), reliable regime signals do not guarantee positive alpha.
...
Standard errors calculated using \citet{Lo2002} methodology adjusted for return non-normality. Ninety-five percent confidence intervals:
\begin{itemize}
...
This section demonstrates the \textbf{central result} of the thesis: market segmentation based on correlation homogeneity is critical for TDA-based regime detection. Prior work \citep{Gidea2018, Meng2021} computed topology on market-wide baskets, which our results show produces unstable features.
...
The pre-verification estimates suggested topology detected \textbf{regime structure} (identifying when volatility structure shifts) rather than \textbf{directional alpha} (predicting which stocks will outperform). Section~\ref{sec:ml} confirms this interpretation: machine learning improves regime classification ($F_1 = 0.58$) but directional prediction remains weak (AUC $\approx 0.52$). The verified backtest (Section~\ref{sec:conclusion}) confirms regime detection alone is insufficient for positive alpha after transaction costs and proper signal lag.
...
Sector-specific topology produces Sharpe $+0.79$ in the initial backtest. However, independent verification using a look-ahead-free implementation, strict 1-day signal lag, and a test window that includes the 2022 bear market yields Sharpe $-0.77$ (Strategy D in \texttt{risk\_report.py}). The discrepancy arises from a favorable test window, larger stock universe, and a subtle same-day signal bias in the original code---common pitfalls in quantitative research \citep{Bailey2014}.

The correlation-stability relationship ($\rho = -0.87$, $R^2 = 0.76$) is genuine and robust: sector homogeneity does stabilize persistent homology features. This structural finding generalizes across sectors and, as Section~\ref{sec:crossmarket} demonstrates, across international market scenarios. What does not generalize is the leap from \textit{stable topology} to \textit{positive alpha}: stable topological features are a necessary but not sufficient condition for a profitable strategy.

This finding addresses Research Question 1 (\textit{Does topology contain tradeable information?}): \textbf{Partially}---topology reliably detects regime structure, but the beta-spread strategy built on that detection does not produce positive returns after transaction costs in out-of-sample testing.
