...
Trading signals derived through Laplacian smoothing on correlation-based graphs following \citet{Kondor2002}. The methodology proceeds as follows:
...
Persistent homology analyzes topological features of correlation structure following the methodology established by \citet{Gidea2018}. Implementation uses Vietoris-Rips filtration on correlation distance metric:
...
Compute Vietoris-Rips persistence diagrams using the ripser library \citep{Tralie2018}. Extract $H_1$ (first homology) features, including loop count (Betti-1) and total persistence. Calculate topology volatility as the 30-day rolling standard deviation of $H_1$ features. Regime classification: periods are classified as unstable if topology volatility exceeds the 75th percentile threshold.
...
\textbf{Walk-forward validation:} Rolling 3-year training windows with 1-year out-of-sample test periods. All parameters, including regime classification thresholds, derived exclusively from training data. This methodology prevents look-ahead bias and data snooping \citep{Bailey2014, dePrado2018}.

\textbf{Transaction costs:} 5 basis points (0.05\%) per trade representing institutional execution costs \citep{Frazzini2018}. Applied to all position changes, including entries, exits, and rebalancing.
...
\textbf{Statistical validation:} Performance metrics evaluated using standard errors calculated via analytical formulas \citep{Lo2002} and verified with bootstrap resampling \citep{Politis1994}. Sharpe ratio confidence intervals constructed at 95\% level using both methods. All hypothesis tests conducted at $\alpha = 0.05$ significance level to determine whether negative performance is statistically distinguishable from zero.

\textbf{Statistical Power Analysis:} For detecting a Sharpe ratio significantly different from zero with test periods of 252--504 days, we achieve statistical power exceeding 0.95 for effect sizes $|SR| > 0.4$ at $\alpha = 0.05$. Given observed Sharpe ratios of $-0.56$ to $-2.08$, power is effectively 1.0 for detecting these large negative effects. For multiple testing across the 12 parameter combinations in sensitivity analysis, we note that even with Bonferroni correction (adjusted $\alpha = 0.05/12 = 0.0042$), all results remain significant (all $p < 0.001$). Deflated Sharpe ratios per \citet{dePrado2018} accounting for the search over parameters remain negative, confirming results are not artifacts of multiple testing.
