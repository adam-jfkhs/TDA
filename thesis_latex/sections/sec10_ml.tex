...
Sections~\ref{sec:sector}--\ref{sec:variants} demonstrate that sector-specific topology substantially improves regime classification using simple threshold rules (75th percentile cutoffs). However, these rules have critical limitations:
...
\begin{enumerate}
    ...
    \item \textbf{Machine learning dramatically improves $F_1$}: 0.014 $\rightarrow$ 0.578 (Neural Network, best-performing model)
        \begin{itemize}
    ...
    \item \textbf{Sharpe improvement exists but modest}: Neural Network achieves Sharpe $+0.47$ (initial estimate) vs sector-specific baseline
        \begin{itemize}
...
Table~\ref{tab:feature-importance} ranks features by predictive importance (Random Forest model).
...
\textbf{Sector-specific (simple thresholds)}: Sharpe $+0.79$ (initial); $-0.77$ (verified)

\textbf{ML-based (Random Forest)}: Sharpe $+0.47$ (initial)
...
Section~\ref{sec:sector} initially reported Sharpe $+0.79$ using simple thresholds (verified at $-0.77$; see Section~\ref{sec:conclusion}). This section shows ML achieves Sharpe $+0.47$ (initial estimate). How to reconcile?

\textbf{Explanation}:
\begin{enumerate}
    \item Section~\ref{sec:sector} exploits \textbf{market segmentation} (compute topology per sector, filter $\rho > 0.5$)
