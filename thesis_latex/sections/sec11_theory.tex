...
\end{equation}
where $q = n/T$ (ratio of stocks to time observations), equivalently $q = 1/Q$ with $Q = T/n$.

\textbf{Example}: For our empirical setting with $Q = T/n = 20$ (e.g., $n = 20$ stocks, $T \approx 400$ days):
\begin{itemize}
\item $q = 1/20 = 0.05$
\item Expected range: $[(1-\sqrt{0.05})^2, (1+\sqrt{0.05})^2] \approx [0.60, 1.50]$
\end{itemize}
...
\midrule
0.3 & 2.14 & 0.48 & \checkmark Yes (weakly structured) \\
...
\begin{itemize}
\item \textbf{Low correlation ($\rho = 0.3$)}: $\lambda_1 \approx 2.14$ exceeds MP upper bound (1.50) $\rightarrow$ weakly structured
...
From random matrix perturbation theory \citep{Tao2011}:
\begin{equation}
...
\textbf{Fitted Constant}: $\alpha = 0.35$ (from Section 7--9 data; see Figure 11.3, which shows 0 violations across 20 sampled points with mean observed CV $\approx 0.45$)
...
\midrule
0.3 & 0.612 & 0.76 & 0.80 \checkmark \\
0.5 & 0.489 & 0.70 & 0.70 \checkmark \\
0.7 & 0.312 & 0.76 & 0.41 \checkmark \\
0.9 & 0.145 & 1.17 & 0.12 \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Result}: \textbf{All observed values respect the bound} (all ratios $< 1$).

\textbf{Interpretation}:
\begin{itemize}
\item Bound is \textbf{tight at intermediate correlation} (ratio $\approx 0.7$--$0.8$ at $\rho \in [0.3, 0.5]$), looser at extremes
...
Setting $\text{SNR} = 1.5$ and $\alpha = 0.35$ (empirically fitted):
\begin{align}
\frac{\sqrt{\rho(1-\rho)}}{0.35} &\geq 1.5 \\
\sqrt{\rho(1-\rho)} &\geq 0.525 \\
\rho(1-\rho) &\geq 0.276 \quad \text{(impossible, max = 0.25 at $\rho = 0.5$)}
\end{align}
...
\quad Financials & 0.61 & 0.38 & Stable topology (verified Sharpe negative) \\
\quad Energy & 0.60 & 0.40 & Stable topology (verified Sharpe negative) \\
\quad Technology & 0.58 & 0.45 & Stable topology (verified Sharpe negative) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Prior Work}:
\begin{itemize}
\item \citet{Laloux1999}: Eigenvalue cleaning for covariance estimation
\item \citet{Potters2020}: \textit{A First Course in Random Matrix Theory} (eigenvalue spectra)
\item \citet{Tao2011}: Random matrix perturbation theory
\end{itemize}
...
\textbf{Prior Work}:
\begin{itemize}
\item \citet{Fiedler1973}: Algebraic connectivity and graph partitioning
\item \citet{Chung1997}: \textit{Spectral Graph Theory} (Laplacian eigenvalues)
\item \citet{VonLuxburg2007}: Tutorial on spectral clustering
\end{itemize}
...
\textbf{Extension}: Incorporate \textbf{generalized MP laws} for power-law distributed data \citep{Burda2004}.
...
\item \textbf{Theoretical Bound} (Novel Contribution)
   \begin{itemize}
   \item $\text{CV}(H_1) \leq 0.35 / \sqrt{\rho(1-\rho)}$
   \item All empirical observations respect the bound (0 violations across 20 sampled points; Figure 11.3)
...
\item \textbf{Epps Effect in Intraday Data}: Section~\ref{sec:intraday} uses 5-minute bars for high-frequency topology. The Epps effect \citep{Epps1979} causes correlations to decline spuriously at high frequencies due to non-synchronous trading---stocks don't trade in identical milliseconds. This biases $\rho$ downward, potentially inflating observed CV. Applying the \citet{HayashiYoshida2005} estimator, which corrects for asynchronicity, would likely \textit{increase} intraday correlations and further reduce topology noise beyond the reported 32\% improvement.
\end{enumerate}
