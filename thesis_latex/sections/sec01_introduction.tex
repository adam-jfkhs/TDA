...
\subsubsection{Main Result: No Strategy Produced Positive Returns — But the Mathematical Framework Is Real}

\textbf{Empirical Discovery} (Sections 4, 7, verified in \texttt{risk\_report.py}):
\begin{itemize}
\item \textbf{Cross-sector topology fails}: Mixing tech, energy, healthcare stocks produces unstable topology (CV = 0.68), Sharpe $-0.56$
\item \textbf{Sector-specific approach improves substantially}: Computing topology separately per sector yields stable features (CV = 0.40) and Sharpe $-0.77$ (verified) --- much better, but still negative
\item \textbf{All strategies fail to produce positive alpha} after proper signal lag, transaction costs, and Fama-French factor regression
\end{itemize}

\textbf{The Mechanism} (Sections 7, 10, 11 — structurally valid independent of alpha):
\begin{itemize}
\item High within-sector correlation ($\rho > 0.6$) $\rightarrow$ eigenvalue concentration $\rightarrow$ stable topology $\rightarrow$ measurable regime signals
\item Cross-sector mixing ($\rho \approx 0.4$) $\rightarrow$ eigenvalue dispersion $\rightarrow$ unstable topology $\rightarrow$ noisy signals
\item Stable topology reduces strategy losses (Fiedler filter: Sharpe $-2.75 \rightarrow -2.57$) but cannot overcome the absence of a profitable core signal
\end{itemize}

\textbf{Why This Matters}:
\begin{itemize}
\item Rigorous negative result with factor regression: topology does not generate alpha (factor $\alpha$ t-stats: $-5.1$ to $-5.7$)
\item Identifies \textbf{boundary conditions} for topology stability: $\rho > 0.5$ necessary but not sufficient
\item Demonstrates how look-ahead bias and favorable test windows can inflate apparent strategy performance
\end{itemize}
...
\begin{itemize}
\item \citet{Gidea2018}: TDA for crash detection (AUC not reported, no trading strategy)
\item \citet{Meng2021}: Network topology in Chinese markets (descriptive, no profitability test)
\item \citet{Macocco2023}: TDA + ML for crypto (limited validation)
\end{itemize}

\textbf{Our Contribution}:
\begin{enumerate}
\item \textbf{First rigorous factor-regression verification of TDA trading strategies} (all approaches negative after look-ahead-free backtest; best verified Sharpe $-0.77$)
...
\item \textbf{Result}: Sharpe $-0.56 \rightarrow -0.77$ (verified); initial estimate of $+0.79$ inflated by test-window and signal-lag bias
...
\item \textbf{Academic Researchers} (Finance, Applied Math, Data Science)
   \begin{itemize}
   \item Contributes first rigorous negative-result study of TDA trading with factor regression
...
7 & Sector-Specific Topology & 12 & Sharpe $-0.77$ (verified); initial estimate $+0.79$ inflated by test-window and signal-lag bias \\
