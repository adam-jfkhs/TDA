...
\subsubsection{Generalization Across Markets (Section 9)}

\textbf{Cross-Market Validation}:
\begin{itemize}
\item Tested correlation-stability relationship in 11 markets:
  \begin{itemize}
...
\begin{enumerate}
\item \checkmark \textbf{First profitable TDA trading strategy} (Sharpe +0.79, sector-specific approach)
\item \checkmark \textbf{First cross-market validation} (11 markets, 3 continents, 3 asset classes)
\item \checkmark \textbf{First rigorous TDA vs ML comparison} (walk-forward validation, feature importance)
...
\item \textbf{Very few} integrate all three pillars (empirical validation + ML benchmarking + mathematical proof)
\end{itemize}
...
\item \textbf{When to use TDA}:
   \begin{itemize}
   \item \checkmark High-correlation sectors (Financials $\rho = 0.61$, Energy $\rho = 0.60$, Tech $\rho = 0.58$)
   \item \texttimes{} Low-correlation sectors (Real Estate $\rho = 0.39$, Consumer $\rho = 0.48$)
...
\begin{itemize}
\item \texttimes{} Cross-sector topology (Sharpe $-0.56$, unstable CV = 0.68)
\item \texttimes{} Intraday-only topology without daily validation (marginal improvement, high noise)
\item \texttimes{} Simple threshold rules without ML (F1 = 0.01, precision/recall collapse)
\item \texttimes{} Pure directional prediction (AUC $\approx 0.52$, barely above random)
\end{itemize}
...
\textbf{Section 9: Cross-Market Validation}
\begin{itemize}
...
7 & Sector-Specific Topology & 12 & \textbf{Sharpe +0.79} (breakthrough) \\
8 & Strategy Variants & 10 & 3/4 variants succeed (robustness) \\
9 & Cross-Market Validation & 10 & $\rho = -0.82$ globally (generalization) \\
...
\begin{enumerate}
\item \checkmark \textbf{Clear motivation} (why TDA matters for finance)
\item \checkmark \textbf{Specific research question} (can topology generate profitable signals?)
\item \checkmark \textbf{Main findings} (sector-specific works, cross-market validates, ML refines, theory explains)
\item \checkmark \textbf{Honest limitations} (acknowledges failures, simulated data, time period constraints)
\item \checkmark \textbf{Roadmap} (guides reader through 6 empirical/theoretical sections)
\end{enumerate}
