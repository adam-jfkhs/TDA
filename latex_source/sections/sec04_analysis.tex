...
\subsection{Looking Forward: From Failure Analysis to Systematic Investigation}

The preceding analysis has identified specific failure modes---scale mismatch, sample limitations, and market regime misalignment---that explain the strategy's underperformance. However, the 50\% loss reduction achieved by the topological filter suggests that persistent homology captures genuine market structure, even when the trading logic fails to exploit it profitably. This observation raises a critical question: \textit{under what conditions might topological methods succeed?}

The following section consolidates preliminary conclusions from this failure analysis and outlines a systematic research program to address each identified limitation. Rather than abandoning topological methods based on this initial negative result, we pursue a structured investigation spanning six research phases---from intraday data analysis to theoretical foundations---that ultimately transforms the failure into actionable insights about when, where, and why TDA-based strategies can achieve positive risk-adjusted performance.
