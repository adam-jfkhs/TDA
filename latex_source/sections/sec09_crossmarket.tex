...
\begin{tcolorbox}[colback=red!5, colframe=red!70, title=\textbf{IMPORTANT: Simulation Disclaimer}]
\textbf{This entire section uses CALIBRATED SIMULATIONS, not real market data.}

Due to data-access constraints, international equity and cryptocurrency analyses use synthetic correlation matrices calibrated to reproduce empirical characteristics from academic literature. These results are \textbf{illustrative demonstrations} of how the theoretical framework \textit{might} generalize---they are \textbf{NOT live-market backtests}.

\textbf{What this section shows:} The $\rho$-CV relationship pattern holds in simulated scenarios.

\textbf{What this section does NOT show:} Whether real international markets exhibit this pattern.

Validation on actual international market data is left to future work.
\end{tcolorbox}
...
If the correlation-stability relationship from Section 7 ($\rho = -0.95$ between mean correlation and topology CV) holds in simulated international scenarios, this suggests:
...
\begin{itemize}
\item U.S. Sectors alone: $\rho = -0.95$
...
This is \textbf{statistically indistinguishable} from the U.S.-only result ($\rho = -0.95$), confirming that:
