...
where $\alpha = 0.5$ is the diffusion strength parameter, and $x$ is the vector of asset returns. The superscript 3 denotes matrix exponentiation (three iterations of the diffusion process), not matrix transpose.\footnote{Standard notation would use $T$ for iterations, but we use the explicit value 3 to avoid confusion with the transpose operation.} Residuals are calculated as:
