...
\toprule
\textbf{Strategy} & \textbf{Sharpe} & \textbf{Ann. Ret.} & \textbf{Max DD} & \textbf{Win Rate} & \textbf{Active} \\
\midrule
...
\textbf{Note}: All results from walk-forward out-of-sample testing (January 2023--December 2024, 512 trading days). Transaction costs of 5 bps applied.
...
\textbf{Figure 8.1}: Equity curves and drawdown comparison available in supplementary materials (GitHub repository).
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Regime} & \textbf{Days} & \textbf{Mom+TDA Ret.} & \textbf{Baseline Ret.} & \textbf{Diff.} \\
\midrule
High $H_1$ (Stressed) & 78 (15\%) & +0.12\%/day & +0.09\%/day & +33\% \\
Low $H_1$ (Calm) & 434 (85\%) & +0.04\%/day & $-0.01\%$/day & +500\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Figure 8.2}: Performance comparison across variants available in supplementary materials.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Portfolio} & \textbf{Sharpe} & \textbf{Ann. Ret.} & \textbf{Max DD} & \textbf{Avg. Corr.} \\
\midrule
Best Individual (Momentum + TDA) & 0.42 & 2.8\% & $-14.2\%$ & N/A \\
Ensemble (Equal-Weight) & 0.48 & 3.1\% & $-12.8\%$ & 0.38 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Figure 8.3}: Ensemble vs. individual strategy comparison available in supplementary materials.
...
Phase 2 (Section~\ref{sec:sector}) & Sharpe +0.79 & Sector-specific (homogeneity) \\
Phase 3 (Section~\ref{sec:variants}) & Sharpe +0.48 & Strategy variants (robustness) \\
\bottomrule
\end{tabular}
\end{table}
